...
app.put("/", async (req, res) => {
  ...
  try {
    ...
    const form = await FormRegister.findById(data._id);
    if (form.status !== "Chưa duyệt") {
      res.send({
        status: "ok",
        message: "Thành công",
        data: form,
      ...
      if (type === "denied") {
        ...
        await smtp.sendMail({
          ...
          from: adminEmail,
          subject:
            "GoTruck - Thông báo về đơn đăng ký trở thành đối tác tài xế", // Tiêu đề email
        ...
        const shipperNew = {
          ...
          current_address: {
            address:
              "RMCQ+72W, Phường 4, Gò Vấp, Thành phố Hồ Chí Minh, Vietnam",
